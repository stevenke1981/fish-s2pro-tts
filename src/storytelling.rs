...
/// Parse actual input tags for QA without adding director recommendations.
pub fn parse_story_script(text: &str) -> Vec<StoryLine> {
    text.lines().map(|raw| {
        let mut remaining = raw.trim();
        let mut tags = Vec::new();
        while remaining.starts_with('[') {
            let Some(end) = remaining.find(']') else { break; };
            tags.push(remaining[1..end].to_string());
            remaining = remaining[end + 1..].trim_start();
        }
        StoryLine {
            speaker: "旁白".to_string(), is_narrator: true, tags,
            text: remaining.to_string(), pause_after_ms: 350,
            intensity: EmotionIntensityLevel::Level2Normal,
        }
    }).collect()
}
...
pub fn auto_direct_story(text: &str, preset: &StoryPreset) -> Vec<StoryLine> {
    ...
    // 分行處理
    for parsed in parse_story_script(cleaned) {
        let line = parsed.text.trim();
        if line.is_empty() {
            continue;
        }
        ...
        let mut merged_tags = parsed.tags;
        for tag in tags { if !merged_tags.contains(&tag) { merged_tags.push(tag); } }
...
pub fn qa_check_story_script(lines: &[StoryLine]) -> Vec<ScriptQaIssue> {
    ...
    for (idx, line) in lines.iter().enumerate() {
        ...
        for ch in line.text.chars() {
            if ch == '[' { depth += 1; if depth > 1 { malformed = true; } }
            if ch == ']' { depth -= 1; if depth < 0 { malformed = true; } }
        }
        if depth != 0 || malformed {
            issues.push(ScriptQaIssue { line_index: idx, severity: ScriptQaSeverity::Warning,
                message: format!("第 {} 行方括號標籤未正確閉合", idx + 1),
                suggested_fix: "請檢查 [ 與 ] 是否成對，並避免巢狀標籤。".to_string() });
        }
