...
impl TimelineClip {
    ...
    pub fn new_mock(
        ...
        for i in 0..total_samples {
            let phase = (i as f32 * 440.0 * 2.0 * std::f32::consts::PI) / 44100.0;
            pcm.push((phase.sin() * 12000.0) as i16);
        }
        let waveform_peaks = Self::calculate_waveform_peaks(&pcm, channels, 120);
...
impl TimelineState {
    ...
    pub fn synthesize_clip_speech(
        ...
        let req = SpeechRequest {
            ...
            speed: Some(clip.speed.clamp(0.5, 2.0)),
        };
    ...
    pub fn start_playback(&mut self, audio_player: &mut AudioPlayer) {
        ...
        match self.mix_timeline_to_pcm() {
            Ok((pcm, sr, ch)) => {
                let wav_bytes = encode_pcm_to_wav(&pcm, sr, ch);
                if audio_player.is_device_available() {
                    if let Ok(()) = audio_player.play_bytes(wav_bytes) {
                        if self.playhead_sec > 0.05 {
                            let _ = audio_player.seek(Duration::from_secs_f32(self.playhead_sec));
                        }
                ...
                .show(ui, |ui| {
                    ui.horizontal(|ui| {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                            ...
                            let can_synth = !is_synth && !clip.text.trim().is_empty();
    ...
    if state.show_tts_modal {
        egui::Window::new("🎙️ 新增 Fish Audio TTS 語音片段至時間軸")
            ...
            .show(ui.ctx(), |ui| {
                ...
                ui.horizontal(|ui| {
                    let can_gen = !state.tts_modal_is_generating && !state.tts_modal_text.trim().is_empty();
