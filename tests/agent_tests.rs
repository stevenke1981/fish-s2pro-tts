...
#[test]
fn cli_mcp_process_handshake_and_tool_call_use_clean_json_lines() {
    use std::io::Write;
    use std::process::Stdio;
    let dir = TempDir::new();
    let mut child = Command::new(env!("CARGO_BIN_EXE_fish-s2pro-tts-cli"))
        .env_remove("OPENROUTER_API_KEY")
        .args(["--output-dir"]).arg(&dir.0).args(["mcp", "--stdio"])
        .stdin(Stdio::piped()).stdout(Stdio::piped()).stderr(Stdio::piped())
        .spawn().unwrap();
    let mut input = child.stdin.take().unwrap();
    for message in [
        json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"2025-11-25","capabilities":{},"clientInfo":{"name":"process-test","version":"1"}}}),
        json!({"jsonrpc":"2.0","method":"notifications/initialized"}),
        json!({"jsonrpc":"2.0","id":2,"method":"tools/list"}),
        json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"fish_prepare","arguments":{"text":"測試台詞","output":"test.mp3"}}}),
    ...
    let replies: Vec<Value> = text.lines().map(|line| serde_json::from_str(line).unwrap()).collect();
    assert_eq!(replies.len(), 3);
    assert_eq!(replies[1]["result"]["tools"].as_array().unwrap().len(), 7);
    assert_eq!(replies[2]["result"]["structuredContent"]["request"]["input"], "測試台詞");
    assert_eq!(std::fs::read_dir(&dir.0).unwrap().count(), 0);
}
