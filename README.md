...
8. **深層故事演繹與智慧導演 (Storytelling Studio)**：
   - 整合 `fish-audio-s2.1-pro-storytelling` 專業體系，涵蓋 6 大主流故事體裁（懸疑推理、恐怖怪談、玄幻武俠、浪漫治癒、少兒睡前繪本、科幻史詩）與 10 大導演標籤系統（氣息呼吸、距離遠近、重音與節奏節拍、語速突變等）。
   - **🎭 智慧自動導演 (Auto-Director)**：一鍵分析長篇故事台詞的情境脈絡，自動推斷並穿插最佳的情緒音色與導演情緒標籤。
...
9. **剪映 / Filmora 風格專業多軌時間軸編輯器 (Timeline Editor)**：
   ...
   - **直接 TTS 生成與多軌混音匯出**：選取 Clip 即可直接呼叫 TTS 重新合成，並支援將多軌道整體非破壞渲染混音為高品質立體聲 WAV / MP3 檔案。
...
└── tests/
    └── integration_tests.rs    # 端到端自動化測試 (21 項整合測試)
```
